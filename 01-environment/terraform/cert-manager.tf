resource "null_resource" "install_cert_manager" {
  ...
  triggers = {
    ...
    cert_manager_ingress_class_name           = var.cert_manager_ingress_class_name
    ...
    cert_manager_ingress_gateway_namespace    = var.cert_manager_ingress_gateway_namespace
    cert_manager_ingress_gateway_service_name = var.cert_manager_ingress_gateway_service_name
    cert_manager_manifest_hash                = local.cert_manager_manifest_hash
    cert_manager_ingress_class_hash           = local.cert_manager_ingress_class_hash
  ...
  provisioner "local-exec" {
    ...
    environment = {
      ...
      CERT_MANAGER_INGRESS_CLASS_NAME           = self.triggers.cert_manager_ingress_class_name
      ...
      CERT_MANAGER_INGRESS_GATEWAY_NAMESPACE    = self.triggers.cert_manager_ingress_gateway_namespace
      CERT_MANAGER_INGRESS_GATEWAY_SERVICE_NAME = self.triggers.cert_manager_ingress_gateway_service_name
    }
  }
