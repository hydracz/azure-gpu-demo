resource "azurerm_monitor_data_collection_rule" "prometheus" {
  name                        = "MSProm-${var.location}-${azurerm_kubernetes_cluster.main.name}"
  resource_group_name         = azurerm_resource_group.main.name
  location                    = var.location
  description                 = "DCR for Azure Monitor Metrics Profile (Managed Prometheus)"
  data_collection_endpoint_id = azurerm_monitor_workspace.main.default_data_collection_endpoint_id
  kind                        = "Linux"

  destinations {
    monitor_account {
      monitor_account_id = azurerm_monitor_workspace.main.id
      name               = "prometheus-${azurerm_monitor_workspace.main.name}"
    }
  }

  data_flow {
    streams      = ["Microsoft-PrometheusMetrics"]
    destinations = ["prometheus-${azurerm_monitor_workspace.main.name}"]
  }
...
resource "azurerm_monitor_alert_prometheus_rule_group" "node_recording_rules" {
  name                = "NodeRecordingRulesRuleGroup-${azurerm_kubernetes_cluster.main.name}"
  location            = var.location
  resource_group_name = azurerm_resource_group.main.name
  cluster_name        = azurerm_kubernetes_cluster.main.name
  description         = "Node Recording Rules Rule Group"
  rule_group_enabled  = var.prometheus_rule_group_enabled
  interval            = var.prometheus_rule_group_interval
  scopes              = [azurerm_monitor_workspace.main.id, azurerm_kubernetes_cluster.main.id]
...
resource "azurerm_monitor_alert_prometheus_rule_group" "kubernetes_recording_rules" {
  name                = "KubernetesRecordingRulesRuleGroup-${azurerm_kubernetes_cluster.main.name}"
  location            = var.location
  resource_group_name = azurerm_resource_group.main.name
  cluster_name        = azurerm_kubernetes_cluster.main.name
